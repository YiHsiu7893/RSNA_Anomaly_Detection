...
gen_weights_path =  'model_weight/generator/generator_weights_20.pth' 
if os.path.exists(gen_weights_path):
    gen.load_state_dict(torch.load(gen_weights_path))
    print("use pretrained weight")
dis_weights_path =  'model_weight/discriminator/discriminator_weights_20.pth' 
if os.path.exists(dis_weights_path):
    discrim.load_state_dict(torch.load(dis_weights_path))
    print("use pretrained weight")
...
for epoch in range(epochs):
  ...
  show_and_save('noise_picture_epoch_%d.png' % (epoch + 21) ,make_grid((c*0.5+0.5).cpu(),8))
  show_and_save('reconstruct_picture_epoch_%d.png' % (epoch + 21) ,make_grid((b*0.5+0.5).cpu(),8))
  ...
  if not os.path.exists(discriminator_weights_folder):
      ...
      print("make dir")    
  generator_weight_path = os.path.join(generator_weights_folder, f'generator_weights_{epoch + 21}.pth') 
  discriminator_weight_path = os.path.join(discriminator_weights_folder, f'discriminator_weights_{epoch + 21}.pth') 
  torch.save(gen.state_dict(), generator_weight_path)
  torch.save(discrim.state_dict(), discriminator_weight_path)
